...
; groupPic : ListOfPerson -> Image
; GIVEN: takes a list of Person
; RETURNS: an image, like a group picture, of the persons in the list
; EXAMPLES:
#|
(groupPic (list 
           (make-person "John" "Marshal" 45 80 40)
           (make-person "Henry" "Stinson" 25 80 20)
           (make-person "Jim" "Parson" 10 40 20)
           (make-person "George" "Micheal" 42 160 40)))
|#

(define (groupPic listOfPersons)
  (cond
    [(empty? listOfPersons) "Enter a non-empty list"]
    [(empty? (rest listOfPersons)) (person-image (first listOfPersons))]
    [else (beside/align "bottom"
                        (person-image (first listOfPersons))
                        (groupPic (rest listOfPersons)))]))